...
class AllTest(unittest.TestCase):
    ...
    def createa_admin_user(self):
        new_user = User(
            name='Superman',
            email='admin@realypthon.com',
    ...
    def test_logged_in_usesrs_can_access_tasks_page(self):
        self.register(
    ...
    def test_user_can_complete_tasks(self):
        ...
        self.assertIn(b'The task is comeplete. Nice.', response.data)
    ...
    def test_users_cannot_complete_tasks_that_are_not_created_by_them(self):
        ...
        self.create_tasks()
        self.logout()
        self.create_user('Fletcher', 'fletcher@realpyhton.com', 'python101')
        self.login('Fletcher', 'python101')
        self.app.get('tasks/', follow_redirects=True)
        response = self.app.get("complete/1/", follow_redirects=True)
        self.assertIn(
    ...
    def test_admin_users_can_complete_tasks_that_are_not_created_by_them(self):
        ...
        response = self.app.get('complete/1/', follow_redirects=True)
        self.assertNotIn(
            b'You can only update tasks that beong to you.', response.data
        )
        ...
        def test_admin_users_can_delete_tasks_that_are_not_created_by_them(self):
            ...
            self.create_tasks()
            self.logout()
            self.createa_admin_usere()
            self.login('Superman', 'allpowerful')
            self.app.get('tasks/', follow_redirects=True)
            response = self.app.get('delete/1/', follow_redirects=True)
            self.assertNotIn(
                b'You can only delete tasks that beong to you.', response.data
            )
