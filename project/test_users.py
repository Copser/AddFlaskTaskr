...
class UsersTest(unittest.TestCase):
    """ Building users test, startin whit configuration
        setup and teardown, then helper methods and at the
        end writing users tests."""
    ...
    def create_task(self):
        return self.app.post('add/', data=dict(
            ...
            due_date='10/04/2015',
            priority='1',
            posted_date='10/04/2015',
            status='1'
        ), follow_redirects=True)

    # Starting whit user testing
    ...
    def test_invalid_form_data(self):
        self.register('Michael', 'michael@realpython', 'python', 'pthon')
        response = self.login('alset("alert box!")', 'foo')
    ...
    def test_user_login_field_errors(self):
        response = self.app.post('/', data=dict(name='',
                                 password='python101',),
                                 follow_redirects=True)
        self.assertIn(b'This field is required.', response.data)

    def test_string_representation_of_the_user_object(self):
        db.session.add(User("Johnny", "jonn@doe.com", "johnny"))
        db.session.commit()
    ...
    def test_default_user_role(self):
        db.session.add(User("Johnny", "john@doe.com", "johnn"))
        db.session.commit()
        users = db.session.query(User).all()
        for user in users:
